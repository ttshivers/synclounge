# build environment
FROM --platform=$BUILDPLATFORM node:16.2.0-alpine3.12 as build-stage
...
FROM node:16.2.0-alpine3.12 as dependency-stage
RUN mkdir /app && chown -R node:node /app
WORKDIR /app
RUN apk add --no-cache python3 make g++
USER node
...
FROM node:16.2.0-alpine3.12 as production-stage
